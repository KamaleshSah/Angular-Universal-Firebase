...
export class MetafrenzyService {
    ...
    setOpenGraph(og: OpenGraph) {
        Object.keys(og).forEach((name: string) => {
            console.log('og:' + name, String(og[name]));
            this.setMetaTag('og:' + name, String(og[name]));
        });
    }
    setOpenGraphForImage(og: OpenGraph) {
        Object.keys(og).forEach((name: string) => {
           // console.log('og:width' + name, String(og[name]));
            this.setMetaTag('og:image:' + name, String(og[name]));
            this.setMetaTag('og:image:' + name, String(og[name]));
        });
    }

    setOpenGraphForTwitter(og: OpenGraph) {
        Object.keys(og).forEach((name: string) => {
           // console.log('og:width' + name, String(og[name]));
            this.setMetaTag('twitter:' + name, String(og[name]));
